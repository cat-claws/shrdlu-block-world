...
from shrdlu_blocks.simulator.scenes import Scene, make_standard_scene
...
class ShrdluBlocksEnv:
    ...
    def reset(self) -> Dict[str, object]:
        ...
        with self._lock:
            ...
            self._controller = Controller(self._scene)
            self._record_event('reset', 'reset', 'Environment reset.', True)
    ...
    def execute_action(self, action: Dict[str, object]) -> Optional[str]:
        ...
        label = self._format_action_label(name, args)
        with self._lock:
            try:
                if name == 'finish':
                    result = None
                ...
                elif name == 'move_grasper':
                    x = self._require_float(args, 'x')
                    y = self._require_float(args, 'y')
                    result = self._controller_result(self._controller.move_grasper(x, y))
                elif name == 'lower_grasper':
                    result = self._controller_result(self._controller.lower_grasper())
                elif name == 'raise_grasper':
                    result = self._controller_result(self._controller.raise_grasper())
                elif name == 'close_grasper':
                    result = self._controller_result(self._controller.close_grasper())
                elif name == 'open_grasper':
                    result = self._controller_result(self._controller.open_grasper())
                elif name == 'highlight_object':
                    obj_id = self._require_int(args, 'obj_id')
                    result = self._controller_result(self._controller.highlight_object(obj_id))
                elif name == 'unhighlight_object':
                    obj_id = self._require_int(args, 'obj_id')
                    result = self._controller_result(self._controller.unhighlight_object(obj_id))
                else:
                    raise ValueError('Unsupported action: %s' % name)
            except Exception as exc:
                self._record_event('action', label, 'ERROR: %s' % exc, False)
                raise
            if name != 'finish':
                self._record_event('action', label, result or 'OK', True)
            return result
    ...
    def event_log(self, limit: int = 50) -> List[Dict[str, object]]:
        """Return the most recent simulator events."""
        with self._lock:
            return [dict(item) for item in self._event_log[-int(limit):]]

    def _record_event(self, kind: str, label: str, result: str, ok: bool) -> None:
        ...
        if len(self._event_log) > 200:
            del self._event_log[:-200]

    @staticmethod
    def _format_action_label(name: str, args: object) -> str:
        if not isinstance(args, dict) or not args:
            return name
        return '%s %s' % (name, json.dumps(args, sort_keys=True))
